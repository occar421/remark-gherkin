...
- [ ] 各 `intentional divergence` に、妥当な Markdown-with-Gherkin が `.feature` の解析動作を完全には再現できない、または再現すべきでない理由が記載されている。
...
- [ ] `gherkin-lint` の固定コミットに含まれる各対応ルールのテストファイルを読み取り、テストケース名、入力、設定、期待メッセージ、期待件数、位置情報をケース単位の移植台帳に登録する。
...
- [ ] 各ルールの移植完了時にケース台帳と `tests/index.test.ts` の対応を確認し、各ケースが実行されることを確認する。
- [ ] 各修正バッチ後に `vp check` と `vp run -r test` を実行し、完了報告前に再帰的な全テストスイートを実行する。
...
- [ ] 固定した upstream テストをケース台帳へ分解し、ルール名、元テスト、ケース名、設定、入力、期待値、移植先テストをチェックボックスで列挙する。
- [ ] 各対応パッケージの `tests/index.test.ts` に、upstream の全正常系・異常系・境界値・オプションケースを Markdown-with-Gherkin fixture として実装する。
